...
function T3TriangleArrows() {
  return (
    <svg
      ...
      fill="none"
    >
      <line x1="43.4" y1="22.6" x2="24.2" y2="56.8" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <line x1="21.6" y1="55.4" x2="40.8" y2="21.2" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <line x1="59.2" y1="21.2" x2="78.4" y2="55.4" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <line x1="75.8" y1="56.8" x2="56.6" y2="22.6" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <line x1="31" y1="68.5" x2="69" y2="68.5" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <line x1="69" y1="71.5" x2="31" y2="71.5" stroke="white" strokeWidth="0.6" strokeOpacity="0.45" strokeDasharray="2.5 2" />
      <circle cx="50" cy="49" r="13" stroke="white" strokeOpacity="0.2" strokeWidth="0.5" strokeDasharray="2 2" />
...
/* ── Tab: How we create impact (intro poster) ───────────────────── */
export function ImpactIntro({ content }: ImpactProps) {
  const intro = useFadeIn();

  return (
    <section
      className="w-full bg-[#FAFAFA] py-16 sm:py-24 overflow-hidden"
      style={{ width: "100vw", marginLeft: "calc(-50vw + 50%)" }}
    >
      <div
        ref={intro.ref}
        className={`mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 relative transition-all duration-700 ease-out ${intro.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      >
        {/* Background Image Block (Shifted Right) */}
        <div className="relative w-full lg:w-[65%] lg:ml-auto aspect-[4/3] rounded-[2rem] sm:rounded-[3rem] overflow-hidden shadow-2xl border border-black/5 group">
          {content.imagePath && (
            <Image
...
/* ── Tab: Transforming barriers into lasting impact ─────────────── */
export function ImpactBarriers({ content }: ImpactProps) {
  ...
  return (
    ...
    >
      <div
        ref={challenges.ref}
        className={`mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 transition-all duration-700 ease-out ${challenges.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      >
...
/* ── Tab: The 3T model in action ────────────────────────────────── */
export function Impact3TModel({ content }: ImpactProps) {
  ...
  return (
    <section
      id="three-t-model"
      ref={model.ref}
      className={`w-full bg-[#141416] pt-16 sm:pt-20 pb-16 sm:pb-24 relative overflow-hidden text-white transition-all duration-700 ease-out ${model.visible ? "opacity-100" : "opacity-0"}`}
      style={{ width: "100vw", marginLeft: "calc(-50vw + 50%)" }}
    >
...
/* ── Tab: Creating systemic impact ──────────────────────────────── */
export function ImpactSystemic({ content }: ImpactProps) {
  ...
  return (
    ...
    >
      <div
        ref={systemic.ref}
        className={`mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 transition-all duration-700 ease-out ${systemic.visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
      >

...
type ClinicWorkflowCircleProps = {
  workflow: OurWorkContent["clinicWorkflow"];
};

const stepColors = [
  { hex: "#5b7bcf", glow: "rgba(91, 123, 207, 0.4)" },
  { hex: "#3b82f6", glow: "rgba(59, 130, 246, 0.4)" },
  { hex: "#f5b82e", glow: "rgba(245, 184, 46, 0.4)" },
  { hex: "#8ce624", glow: "rgba(140, 230, 36, 0.4)" },
  { hex: "#2ec971", glow: "rgba(46, 201, 113, 0.4)" },
  { hex: "#28d2a0", glow: "rgba(40, 210, 160, 0.4)" },
  { hex: "#2e731d", glow: "rgba(46, 115, 29, 0.4)" },
];

export default function ClinicWorkflowCircle({ workflow }: ClinicWorkflowCircleProps) {
    ...
    >
      ...
      <FadeIn delay={0.2} className="w-full hidden sm:block">
        <div className="relative w-full max-w-[40rem] h-[38rem] mx-auto flex items-center justify-center mt-12">
          {/* Rotating Background Dashed Track */}
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-28 h-28 sm:w-44 sm:h-44 rounded-full bg-white shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex items-center justify-center text-center p-4 sm:p-6 border-[6px] sm:border-8 border-primary/10 group transition-transform duration-500 hover:scale-105">
            <div className="space-y-1">
              <div className="w-8 sm:w-12 h-1 bg-primary/30 mx-auto rounded-full mb-1 sm:mb-2 group-hover:w-16 transition-all" />
              <p className="text-[10px] sm:text-base font-black text-[#1a1a1a] [font-family:var(--font-heading)] leading-tight tracking-wide">
                <span className="text-primary text-lg sm:text-3xl drop-shadow-md">Uday</span>
                <br />
                Journey
              </p>
              <div className="w-8 sm:w-12 h-1 bg-primary/30 mx-auto rounded-full mt-1 sm:mt-2 group-hover:w-16 transition-all" />
            </div>
          </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ animationDelay: `${idx * (1.5 / totalSteps)}s`, color: arrowColor }}
                >
          ...
          {allSteps.map((step, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`w-20 h-20 sm:w-32 sm:h-32 rounded-full flex flex-col items-center justify-center p-2 sm:p-4 text-center cursor-pointer relative overflow-visible transition-all duration-300 shadow-lg border-[3px] sm:border-4 border-white ${isActive ? "ring-4 ring-offset-2 scale-105" : "hover:shadow-xl"}`}
                  style={{
                ...
                >
                  ...
                  {step.description && (
                    <div
                      className={`hidden sm:block absolute bottom-full mb-4 left-1/2 -translate-x-1/2 w-72 bg-[#1a1a1a] text-white p-4 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.3)] transition-all duration-500 z-[100] text-left border border-white/10 ${isActive ? "opacity-100 visible translate-y-0" : "opacity-0 invisible translate-y-2 group-hover:opacity-100 group-hover:visible group-hover:translate-y-0"}`}
                    >
                      <div className="flex items-center gap-2 mb-2 border-b border-white/10 pb-2">
                        <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: nodeColor.hex }} />
      ...
      <FadeIn delay={0.2} className="w-full sm:hidden px-4">
        <div className="relative space-y-4">
          <div className="absolute left-[1.35rem] top-4 bottom-4 w-px border-l-2 border-dashed border-primary/20 z-0" />
          {allSteps.map((step, idx) => {
            const isActive = activeIndex === idx;
            const isStart = idx === 0;
            const isEnd = idx === totalSteps - 1;
            ...
            return (
              ...
              >
                <div
                  className={`relative z-10 w-11 h-11 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 shadow-md border-2 border-white ${isActive ? "ring-4 ring-offset-1" : ""}`}
                  style={{
                    backgroundColor: nodeColor.hex,
                    color: nodeColor.hex === "#f5b82e" || nodeColor.hex === "#8ce624" ? "#1a1a1a" : "#ffffff",
                  }}
                ...
                <div
                  className={`flex-1 p-4 rounded-2xl border transition-all duration-300 ${isActive ? "bg-white shadow-[0_10px_30px_rgba(0,0,0,0.08)]" : "bg-white/60 border-gray-200"}`}

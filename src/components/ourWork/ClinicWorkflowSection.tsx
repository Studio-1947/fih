...
export default function ClinicWorkflowSection({ workflow }: ClinicWorkflowSectionProps) {
  ...
  return (
    <section className="relative z-10 mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8 bg-[#faf9f6] rounded-[4rem] my-20 overflow-hidden min-h-[90vh] flex flex-col items-center justify-center border border-black/5 shadow-2xl">
      ...
      <div className="relative w-full max-w-4xl aspect-square mx-auto flex items-center justify-center mt-4 scale-90 sm:scale-100">
        ...
        {arrowAngles.map((angle, idx) => (
          ...
          >
            <div 
              className="text-primary animate-blink-path"
        ...
        {allSteps.map((step, idx) => {
          ...
          return (
            <div
              key={`node-${idx}`}
              className="absolute top-1/2 left-1/2 z-20 transition-transform duration-300 hover:scale-110 group circular-node"
              style={{
                '--angle': `${angle}deg`,
            ...
            >
              <div className={`
                ...
                hover:border-primary hover:shadow-[0_20px_50px_rgba(202,154,34,0.3)]
              `}>
                {isStart ? <LogIn className="w-4 h-4 mb-1" /> : isEnd ? <LogOut className="w-4 h-4 mb-1" /> : <span className="text-[10px] font-black opacity-40 mb-0.5 group-hover:opacity-100 transition-opacity">0{idx}</span>}
                ...
                {!isStart && !isEnd && (
                  <div className="mt-1 sm:mt-2 text-primary opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {/* Tooltip Description */}
                {step.description && (
                  <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 w-52 sm:w-72 bg-[#1a1a1a] text-white p-4 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.3)] opacity-0 group-hover:opacity-100 transition-all pointer-events-none invisible group-hover:visible z-50 text-left border border-white/10">
